...
import { AlertCircle, Clock, ImagePlus, Upload, Video, X } from "lucide-react";
import { useRef, useState } from "react";
import { toast } from "sonner";
import { ExternalBlob } from "../backend";
import UploadSuccessModal from "../components/UploadSuccessModal";
import { useInternetIdentity } from "../hooks/useInternetIdentity";
import { useUploadThumbnail, useUploadVideo } from "../hooks/useQueries";

export default function UploadVideoPage() {
  const navigate = useNavigate();
  const { identity } = useInternetIdentity();
  const { mutate: uploadVideo, isPending: isVideoUploading } = useUploadVideo();
  const { mutateAsync: uploadThumbnail, isPending: isThumbnailUploading } =
    useUploadThumbnail();
  ...
  if (!isAuthenticated) {
    return (
      <div className="max-w-2xl mx-auto text-center py-20">
        ...
        <Button onClick={() => navigate({ to: "/" })}>Go to Video Feed</Button>
      </div>
    );
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="relative w-full rounded-lg overflow-hidden border-2 border-border">
                  ...
                  <div className="px-3 py-2 bg-muted/50 flex items-center justify-between">
                    ...
                    {!isPending && (
                      <label
                        htmlFor="thumbnail"
                        className="cursor-pointer text-sm text-primary hover:underline ml-2 shrink-0"
                      >
                        Change
                      </label>
                    )}
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted/50 rounded-md px-3 py-2">
                ...
                <span>
                  Videos must be between <strong>10 and 20 seconds</strong>{" "}
                  long.
                </span>
              </div>

              <div
                className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${durationError ? "border-destructive" : "border-border hover:border-primary"}`}
              >
                <input
                  ref={fileInputRef}
                  id="video"
            ...
            <Button
              type="submit"
              disabled={
                isPending || !title.trim() || !videoFile || !!durationError
              }
              className="w-full"
              size="lg"
            >
